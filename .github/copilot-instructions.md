...
Current version: **0.16.0** (defined in `spdchk.h`).
...
| File | Purpose |
|---|---|
| `udp.h` | Declares `struct udp_result` (`packets_sent`, `packets_received`, `lost_packets`, `out_of_order`, `jitter_us`, `peak_jitter_us`, `target_bw_mbps`, `achieved_bw_mbps`), `run_udp_client()`, and (under `TEST_MODE`) `udp_preflight_query()` |
| `udp.c` | UDP test client: Phase A TCP negotiation (`SPDCHK_UDP_REQ`), **Preflight check** (sends 3 UDP probes, waits 300 ms, queries server via `SPDCHK_UDP_CHECK` — aborts with a firewall diagnostic if `rx=0`; silently skipped if server is an older version that does not recognise the command), Phase B CBR datagram send loop using `clock_nanosleep`/QPC for sub-millisecond precision, 200 ms pause, Phase C TCP report collection (`SPDCHK_UDP_DONE`); `udp_tcp_connect()` mirrors `connect_timed()` from `client.c`; Phase B loop now aborts on fatal send errors (`WSAECONNRESET`, `ECONNREFUSED`, etc.) so `packets_sent` is not inflated when the server port is unreachable; when `udp_collect_report()` fails, `lost_packets` is set to `packets_sent` for a consistent display; emits a `log_error` when the server confirms 0 of N packets received, pointing the user to check UDP firewall rules; `udp_preflight_query()` is exposed as non-static under `#ifdef TEST_MODE` for unit testing |
| `spdchk.h` | Shared constants (`DEFAULT_PORT`, `DEFAULT_COUNT`, `DEFAULT_DURATION`, `DEFAULT_STREAMS`, `DEFAULT_UDP_BW`, `DEFAULT_PKT_SIZE`, `TEST_MODE_TCP`, `TEST_MODE_UDP`, `SPDCHK_UDP_MAGIC`, DSS parameters), `SPDCHK_VERSION` macro, the packed `spdchk_payload` wire struct (seq number, departure timestamp, optional padding), `spdchk_udp_payload` (seq_number, timestamp_ns, magic_id, flexible padding[]), the `spdchk_report` struct (now includes UDP fields: `lost_packets`, `out_of_order`, `jitter_us`), `SPDCHK_REPORT_REQ`, and the `SPDCHK_UDP_REQ_PREFIX` / `SPDCHK_UDP_DONE_PREFIX` / `SPDCHK_UDP_CHECK_PREFIX` control-channel greeting prefixes |
...
| `server.c` | Passive bandwidth sink — binds a TCP socket, accepts connections in per-thread handlers, performs the version handshake, drains incoming data with per-session byte accumulation, and enforces the optional `max_duration` receive timeout; **UDP support**: `run_server()` additionally opens a `SOCK_DGRAM` socket on the same port and starts a `udp_listener_thread` that validates `magic_id`, checks per-IP authorisation, tracks sequence numbers and computes RFC 3550 jitter; `sess_t` extended with UDP fields (`udp_active`, `udp_seq_max`, `udp_received`, `udp_out_of_order`, `udp_jitter_ns`, `udp_peak_jitter_ns`, timestamps); `sess_udp_authorize()` marks a client IP as UDP-authorised after the TCP handshake; TCP handler detects `SPDCHK_UDP_REQ` (parse parameters, authorise IP, reply `OK\n`), `SPDCHK_UDP_CHECK` (return current `udp_received` count and reset UDP session counters for a clean main-test start), and `SPDCHK_UDP_DONE` (read per-IP UDP stats, reply `SPDCHK_UDP_REPORT <rx> <ooo> <jitter_us> <peak_us>\n`) greetings; `udp_listener_thread` guards the first-packet `udp_seq_max` update with `udp_received == 0` to fix the off-by-one false out-of-order count; Windows `WSAECONNRESET`/`WSAENETRESET`/`WSAEHOSTUNREACH`/`WSAENETUNREACH`/`WSAEHOSTDOWN` from `recvfrom` are treated as continue (not break) so the listener survives transient ICMP errors; on fatal exit the listener resets `g_udp_fd = -1` and closes the socket so subsequent `SPDCHK_UDP_REQ` negotiations get `ERR UDP_UNAVAILABLE` instead of silently succeeding with `rx=0`; packets dropped from unauthorised sources are logged at DEBUG level with the source IP:port (was TRACE with no address); `SPDCHK_UDP_DONE` handler emits an ERROR when `rx=0` and `sent>0` to direct operators to the server firewall |
...
| `interactive.c` | The entire spdchk UI. `TestType` enum: `TEST_ICMP=0`, `TEST_TCP=1`, `TEST_UDP=2`. Client main menu has **7 items**: 0=Run Reachability (ICMP), 1=Run Bandwidth (TCP), 2=Run UDP (Jitter & Loss), 3=Start Server, 4=View Session History, 5=Settings, 6=Exit — both test types are always visible and directly selectable without going through Settings. `AppCtx` retains `udp_test_mode`, `udp_target_bw`, and `udp_pkt_size`; `udp_test_mode` is toggled by the "Test Type" setting but no longer controls main-menu routing (routing is now determined by the item selected). Settings screen has 14 client items (items 10–12: Test Type toggle, UDP Target BW, UDP Pkt Size). History view has a **UDP / Jitter & Loss Results** table alongside ICMP and TCP tables. `render_udp_results()` displays the UDP result screen. `TestResult` carries per-run UDP counters (`udp_sent`, `udp_received`, `udp_lost`, `udp_jitter_us`, `udp_target_bw`, `udp_achieved_bw`). On Windows server startup, prints **two** separate `netsh advfirewall` rules (one for TCP, one for UDP) so both protocols are opened at the configured port. |
...
| `tests/test_udp.c` | Unit tests for `print_udp_metrics()`: no-loss display, loss % computation, µs→ms jitter conversion, Mbps/Gbps capacity threshold, out-of-order line presence/absence, zero-sent no-crash guard, `test_udp_collect_fail_consistent_display` (verifies that when `lost_packets = packets_sent` the count and percentage are consistent), and `udp_preflight_query()` response parsing: server-confirmed probes, zero-rx (port blocked), unrecognised response (old server), TCP connect failure |
| `.github/workflows/release.yml` | GitHub Actions CI/CD pipeline: `test` job runs `make test` on Ubuntu and uploads the report as an artifact; `build-amd64` and `build-arm64` jobs compile the Linux binaries and upload them; `release` job (needs all others) downloads all artifacts, creates a GitHub Release tagged `v{SPDCHK_VERSION}` with a changelog derived from `git log`, and attaches the two Linux binaries plus the test report; a pre-release guard step fails the job if the computed tag already exists in the repo, preventing accidental overwrites of published releases |
