...
Current version: **0.15.4** (defined in `spdchk.h`).
...
| `tests/test_udp.c` | Unit tests for `print_udp_metrics()`: no-loss display, loss % computation, µs→ms jitter conversion, Mbps/Gbps capacity threshold, out-of-order line presence/absence, zero-sent no-crash guard, and `test_udp_collect_fail_consistent_display` (verifies that when `lost_packets = packets_sent` the count and percentage are consistent) |
